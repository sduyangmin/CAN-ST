        ...
        **args
        ...
        if adaptive_embedding_dim > 0:
            self.adaptive_embedding = nn.init.xavier_uniform_(
                nn.Parameter(
                    torch.empty(in_steps, self.node_num, adaptive_embedding_dim)
                )
            )

        if use_mixed_proj:
            self.output_proj = nn.Linear(
                in_steps * self.model_dim, out_steps * self.output_dim
            )
        else:
...
if __name__ == "__main__":
    model = STAEformer(node_num=883, input_dim=3, output_dim=1)
    inp_shape = (12, 883, 3)
    ...
    # Computational complexity:       10.96 GMac
