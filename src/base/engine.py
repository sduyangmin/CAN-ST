...
class BaseEngine:
    ...
    def train_batch(self):
        ...
        for X, label in self._dataloader["train_loader"].get_iterator():
            ...
            pred, statistics_pred = self.model(X, label)
            ...
            bs, len, nodes, dim = label.shape
            y = label.reshape(bs, len, nodes)
            mean = torch.mean(y, dim=(1)).reshape(bs, 1, nodes)
            std = torch.std(y, dim=(1)).reshape(bs, 1, nodes)

            # mean = torch.mean(y, dim=(1,2)).reshape(bs, 1, 1)
            # std = torch.std(y, dim=(1,2)).reshape(bs, 1, 1)
            ...
            # loss = self._loss_fn(pred, label, mask_value) +  beta * F.mse_loss(statistics_pred, station_ture)
            
            # breakpoint()
            loss = self._loss_fn(pred, label, mask_value) #STN
    ...
    def evaluate(self, mode):
        ...
        elif mode == "test" or mode == "shift":
            test_mae = []
            test_mape = []
            test_rmse = []
            print("Check mask value", mask_value)
            for i in range(self.model.fm.horizon):
