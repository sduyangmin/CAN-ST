...
from src.models.norm.CAN import CAN
...
class KrigingEngine:
    ...
    def save_model(self, save_path):
        ...
        filename = "final_model_s{}_{}-{}.pt".format(
            self._seed, self.year, self.year + 1
        )
    ...
    def train_batch(self):
        ...
        for X, label in self._dataloader["train_loader"].get_iterator():
            ...
            if spatial_noise:
                ...
                std = 1
                sem = torch.add(
                    sem, self._to_device(torch.normal(mean, std, sem.shape))
                )
            ...
            norm_x, pre_sta = self.norm_model(X, 'norm')

            pred1, pred2 = self.model(norm_x, sem)
            # pred1, pred2, label1, label2 = self._inverse_transform(
            #     [pred1, pred2, label1, label2], "train"
            # )
            breakpoint()
            ...
            loss = (loss_ob1 * label1.shape[-2] + loss_un * label2.shape[-2]) / (
                label1.shape[-2] + label2.shape[-2]
            )
    ...
    def train(self):
        ...
        for epoch in range(self._max_epochs):
            ...
            self._logger.info(
                message_train_un.format(
                    epoch + 1, mtrain_loss2, mtrain_rmse2, mtrain_mape2
                )
            )
            message_val_ob = "Epoch: {:03d}, Valid Loss1: {:.4f}, Valid RMSE_ob1: {:.4f}, Valid MAPE_ob1: {:.4f}, Valid Time: {:.4f}s"
            self._logger.info(
                message_val_ob.format(
                    epoch + 1, mvalid_loss1, mvalid_rmse1, mvalid_mape1, (v2 - v1)
                )
            )

            message_val_un = "Epoch: {:03d}, Valid Loss3: {:.4f}, Valid RMSE_un: {:.4f}, Valid MAPE_un: {:.4f}"
            self._logger.info(
                message_val_un.format(
                    epoch + 1, mvalid_loss2, mvalid_rmse2, mvalid_mape2
                )
            )
            ...
            if loss < min_loss:
                ...
                self._logger.info(
                    "Val loss decrease from {:.4f} to {:.4f}".format(min_loss, loss)
                )
                min_loss = loss
                wait = 0
            else:
    ...
    def evaluate(self, mode):
        ...
        with torch.no_grad():
            for X, label in self._dataloader[mode + "_loader"].get_iterator():
                ...
                X, label1, label2 = self._to_device(
                    self._to_tensor([X, label1, label2])
                )
                pred1, pred2 = self.model(X, sem)
                pred1, pred2, label1, label2 = self._inverse_transform(
                    [pred1, pred2, label1, label2], mode
                )
        ...
        if mode == "val":
            ...
            loss = (mae1 * labels1.shape[-1] + mae2 * labels2.shape[-1]) / (
                labels1.shape[-1] + labels2.shape[-1]
            )
            ...
            for i in range(self.model.horizon):
                res1 = compute_all_metrics(
                    preds1[:, i, :], labels1[:, i, :], mask_value1
                )
                ...
                res2 = compute_all_metrics(
                    preds2[:, i, :], labels2[:, i, :], mask_value2
                )
                ...
                mae = (res1[0] * num_node_ob + res2[0] * num_node_un) / (
                    num_node_ob + num_node_un
                )
                rmse = (res1[2] * num_node_ob + res2[2] * num_node_un) / (
                    num_node_ob + num_node_un
                )
                mape = (res1[1] * num_node_ob + res2[1] * num_node_un) / (
                    num_node_ob + num_node_un
                )
                res2 = compute_all_metrics(
                    preds2[:, i, :], labels2[:, i, :], mask_value2
            ...
            self._logger.info(
                log.format(np.mean(test_mae1), np.mean(test_rmse1), np.mean(test_mape1))
            )

            log = "Average Test MAE_un: {:.4f}, Test RMSE_un: {:.4f}, Test MAPE_un: {:.4f}"
            self._logger.info(
                log.format(np.mean(test_mae2), np.mean(test_rmse2), np.mean(test_mape2))
            )
